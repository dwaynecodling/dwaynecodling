...
import {dateParse, getReadTime} from "./convinienceHelper";
...
export namespace PostRepository{
    ...
    export async function getAllPosts(sortByDate?:boolean): Promise<IPostEntry[]> {
        let dir = readCache['allPostsInDir'] ?? fs.readdirSync(path.resolve(__dirname,"../live-posts/live-posts/"));
    ...
    export async function getPostFileContentStructure(fileName:string):Promise<IPostEntry>{
        ...
        let filePath = path.resolve(__dirname,"../posts/live-posts/", fileName);
